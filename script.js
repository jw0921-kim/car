...
function assignCars() {
  ...
  const assignQueue = passengers.map(p => {
    const gender = brothers.includes(p) ? "형제" : "자매";
    const eligible = driverList.filter(d => d.flex || d.gender === gender);
    return { name: p, eligible };
  }).filter(p => p.eligible.length > 0);

  for (const p of assignQueue) {
    p.eligible.sort((a, b) => a.list.length - b.list.length)[0].list.push(p.name);
  }
