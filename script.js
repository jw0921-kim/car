...
const brothers = ["이태섭", "김진우", "이동환", "김지석", "전도영", "김권비", "김민호", "남준혁", "최재혁", "김인수", "문지훈", "전호성", "박지웅", "김두현", "박효원",
  "양인수", "김호진", "이상수", "정형기", "박수용", "고병인", "이동길", "정인교", "권세빈", "김용빈", "김승후", "홍정의", "김민성", "하현성", "김진우2", "전은섭",
  "신창현", "임우혁", "정용진", "강민구", "노현준", "양홍열", "김해원", "백진호", "김교식", "이신섭", "조시형", "임향원", "심어진", "오서준", "임주용", "김도건", "이준희"];
...
function toggleState(name, div) {
  if (!state[name]) {
    state[name] = { car: true, flex: false, exclude: false };
  } else if (state[name].car) {
    state[name] = { car: false, flex: true, exclude: false };
  } else if (state[name].flex) {
    state[name] = { car: false, flex: false, exclude: true };
  } else if (state[name].exclude) {
    state[name] = { car: false, flex: false, exclude: false };
  } else {
    state[name] = { car: true, flex: false, exclude: false };
  }
  updateCard(div, name);
...
function updateCard(div, name) {
  ...
  if (s?.exclude) {
    ...
    if (s?.car) div.classList.add("car"); // 초록
    if (s?.flex) div.classList.add("flexible"); // 파랑
  }
}
...
function assignCars() {
  ...
  if (unassigned.size > 0) {
    alert("탑승 인원이 부족합니다. 성별 무관 차량(파랑)으로 전환하거나 제외 인원 조정이 필요합니다.");
  }
}
